...
def main(data_dir:str = typer.Argument(), input_dir: str = typer.Argument("")):
    optimizer = PoseGraphOptimizer()
    dataset = GenericDataset(data_dir)
    # Load poses and add them to the graph with the odometry edges
    gt_poses = np.load(os.path.join(input_dir, "loop_closure", "gt_poses.npy"))
    ...
    with o3d.utility.VerbosityContextManager(o3d.utility.VerbosityLevel.Debug) as cm:
        for ids in closures:
            try:
                pose_2d = ids[3:].reshape(3, 3)
                pose = np.eye(4)
                pose[:2, :2] = pose_2d[:2, :2]
                pose[:2, -1] = pose_2d[:2, -1]
            except ValueError:
                continue
            scan_i = int(ids[0])
            scan_j = int(ids[1])
